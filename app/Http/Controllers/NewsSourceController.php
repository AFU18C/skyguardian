...
class NewsSourceController extends Controller
{
    ...
    public function update(Request $request, NewsSource $newsSource): RedirectResponse
    {
        ...
        if ($request->boolean('autopublish_enabled')
            && ($connectionChanged
                || $newsSource->source_status !== 'available'
                || $newsSource->destination_status !== 'available')) {
        ...
        try {
            ...
            $newsSource->update([
                ...
                'last_error' => $sourceAvailable && $destinationAvailable
                    ? null
                    : 'Проверка источника новостей или группы назначения завершилась ошибкой.',
                'last_checked_at' => now(),
            ]);
            ...
            if ($sourceAvailable && $destinationAvailable) {
                ...
                return back()->with('check_modal', [
                    ...
                    'message' => 'Источник новостей доступен, публикация разрешена. Сообщения будут выходить от имени '.$publishLabel.'.',
                ]);
            }
