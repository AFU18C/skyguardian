...
{
    public function handle(Request $request, Closure $next, TelegramApiPowerStore $store): Response
    {
        ...
        if (! preg_match('/(<details class="card"[^>]*>.*?<h2>Telegram API<\/h2>.*?<div class="list">)(.*?)(<\/div>\s*<\/div>\s*<\/details>)/s', $html, $match)) {
            return $response;
        }

        $index = 0;
        $body = preg_replace_callback('/<div class="row-status">.*?<\/div>/s', function () use (&$index, $apis, $section, $store): string {
            $api = $apis->get($index++);
            if (! $api) {
                return '<div class="row-status"></div>';
            }

            $checked = $store->enabled($section, (int) $api->getKey()) ? ' checked' : '';

            return '<div class="row-status"><label class="api-power-switch" title="Включить или выключить Telegram API">'
                .'<input type="checkbox" data-api-power data-url="/'.$section.'/settings/apis/'.$api->getKey().'/power"'.$checked.' aria-label="Включить или выключить Telegram API">'
                .'<span></span></label></div>';
        }, $match[2]) ?? $match[2];
