...
class AntiCasinoCampaignService
{
    ...
    private const HEADLINES = [
        ...
        '🕳 КАЗИНО КУПУЄ МІСЦЕ У СТРІЧЦІ — ХТО ПЛАТИТЬ ПОТІМ?',
        '🎯 ЦІЛЬ РЕКЛАМИ — НЕ ВАШ ВИГРАШ, А ВАШ ДЕПОЗИТ',
        '📉 ЗАРПЛАТА ЕЛЕКТРИКА — НЕ БОНУСНИЙ БАНК КАЗИНО',
    ];

    private const OPENINGS = [
        'Коли новинний канал роками збирає довіру, а потім веде аудиторію на депозит, він стає рекламним партнером грального бізнесу.',
        'Людина заходить по новини, а отримує заклик внести гроші, забрати фріспіни або повірити у легкий виграш.',
        'Канал отримує оплату за розміщення. Казино отримує гравця. Ризик програшу залишається звичайній людині.',
        'Реклама азартних ігор маскується під дружню пораду: маленький депозит, великий бонус і нібито простий виграш.',
        'Коли поруч із повідомленнями про небезпеку з’являється заклик поповнити казино, довіра перетворюється на товар.',
        'Великі канали називають себе народними. Але чи поведе народне медіа читача туди, де він може програти зарплату?',
        'Казино купує не банер. Воно купує довіру, яку канал зібрав на термінових новинах і потребі людей в інформації.',
        'Підписник бачить знайомий канал і менше сумнівається. Саме тому реклама казино в медіа така небезпечна.',
        'Власник каналу заробляє одразу. Підписник може платити довго — грошима, боргами та конфліктами в родині.',
        'Коли медіа заробляє на переходах у казино, читач має право знати, де новина, а де інтерес рекламодавця.',
    ];

    private const QUESTIONS = [
        'Хто вони після цього: незалежні новинники чи посередники між казино та гаманцем підписника?',
        'Чому прибуток каналу важливіший за ризик, що людина програє гроші?',
        'Чи можна називати себе народним каналом і одночасно продавати народ казино?',
        'Скільки програних зарплат ховається за успішним рекламним розміщенням?',
        'Чому показують бонуси й виграші, але не показують тих, хто втратив гроші?',
        ...
        'Навіщо називати казино розвагою, якщо бізнес тримається на програшах клієнтів?',
        'Чому канал бере гроші гарантовано, а підписнику пропонує лише шанс?',
    ];

    private const CALLS = [
        'Не віддавай зарплату рекламній воронці. Не переходь у казино з новинних каналів.',
        'Побачив рекламу казино — зроби скриншот, збережи посилання та перевір законність.',
        'Новини мають попереджати про ризики, а не підштовхувати до депозиту.',
        'Довіра людей — не товар. Вимагай від каналів відмови від реклами азартних ігор.',
        'Підтримуй медіа, які не заробляють на можливих програшах своєї аудиторії.',
    ];
    ...
    {
        $headline = self::HEADLINES[($index - 1) % 10];
        $opening = self::OPENINGS[intdiv($index - 1, 10) % 10];
        $question = self::QUESTIONS[intdiv($index - 1, 100) % 10];
        $call = self::CALLS[($index - 1) % count(self::CALLS)];
    ...
    private function generatePng(int $index): string
    {
        ...
        $palette = [
            [0, 0, 0], [8, 20, 38], [25, 18, 52], [38, 16, 28],
            [9, 42, 42], [15, 58, 92], [66, 36, 110], [85, 28, 45],
            [255, 205, 40], [20, 20, 24], [70, 75, 84], [226, 230, 235],
            [245, 247, 250], [26, 150, 96], [255, 70, 70], [220, 25, 55],
        ];
        $plte = '';
    ...
    private function drawSymbol(string &$pixels, int $width, int $cx, int $cy, int $symbol, int $color): void
    {
        switch ($symbol) {
            case 0:
                $this->drawDigit($pixels, $width, $cx - 8, $cy - 14, 7, $color, 4);
                break;
            case 1:
                $this->drawCircle($pixels, $width, $cx, $cy, 12, $color);
                break;
            case 2:
                $this->drawTriangle($pixels, $width, $cx, $cy - 14, $cx - 14, $cy + 12, $cx + 14, $cy + 12, $color);
                break;
            case 3:
                $this->drawRect($pixels, $width, $cx - 13, $cy - 8, $cx + 13, $cy + 8, $color);
                break;
            default:
