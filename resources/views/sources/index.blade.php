...
<div class="layout">
    <aside class="sidebar">
        ...
        <a class="nav-item active" href="{{ route('sources.index') }}">Источники</a>
    ...
    <main class="main">
        <header class="topbar">
            <div class="page-title">Источники</div>
            <div class="user">
        ...
        <section class="content">
            <div class="header"><div><h1>Источники сообщений</h1><p>Добавление Telegram-каналов для будущего подключения через MadelineProto.</p></div></div>

            @if(session('status'))<div class="flash">{{ session('status') }}</div>@endif
            @if($errors->any())<div class="errors">{{ $errors->first() }}</div>@endif

            <div class="panel">
                <form method="POST" action="{{ route('sources.store') }}" class="form-grid">
                    @csrf
                    <div class="field"><label>Название</label><input name="name" value="{{ old('name') }}" placeholder="Например: Новости региона" required></div>
                    <div class="field"><label>Тип</label><select name="type"><option value="telegram">Telegram</option></select></div>
            ...
            <div class="panel list">
                @forelse($sources as $source)
                    <details class="source">
                        <summary class="source-head">
                            ...
                            <div class="actions">
                                <form method="POST" action="{{ route('sources.toggle', $source) }}">@csrf @method('PATCH')<button class="button" type="submit">{{ $source->is_active ? 'Отключить' : 'Включить' }}</button></form>
                                <form method="POST" action="{{ route('sources.destroy', $source) }}" onsubmit="return confirm('Удалить источник?')">@csrf @method('DELETE')<button class="button danger" type="submit">Удалить</button></form>
                            </div>
                        </summary>
                        <form method="POST" action="{{ route('sources.update', $source) }}" class="edit">
                            @csrf @method('PUT')
                ...
                @empty
                    <div class="empty">Источники ещё не добавлены.</div>
                @endforelse
            </div>
        </section>
    </main>
