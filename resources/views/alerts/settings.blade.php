...
<div class="grid">
<details class="card">
<summary class="accordion-head"><div class="accordion-title"><div class="accordion-copy"><h2>Технічний акаунт</h2><p>Підключення через Telethon</p></div></div><div class="accordion-meta">@if($settings->last_error)<span class="status-icon error" role="img" aria-label="Помилка" title="Помилка">×</span>@elseif($settings->technical_status === 'connected')<span class="status-icon ok" role="img" aria-label="Підключено" title="Підключено">✓</span>@else<span class="status-icon warn" role="img" aria-label="Не підключено" title="Не підключено">!</span>@endif<span class="accordion-arrow">⌄</span></div></summary>
...
@elseif($authorizationPending)
<form method="POST" action="{{ route('alerts.telegram.confirm') }}">@csrf<div class="field"><label for="telegram_code">Код із Telegram</label><input class="input" id="telegram_code" name="telegram_code" inputmode="numeric" autocomplete="one-time-code" required></div>@if($passwordRequired)<div class="field"><label for="telegram_password">Пароль 2FA</label><input class="input" id="telegram_password" type="password" name="telegram_password" required></div>@endif<div class="actions"><button class="btn" type="submit">Підтвердити</button></div></form>
@else
...
<details class="card">
<summary class="accordion-head"><div class="accordion-title"><div class="accordion-copy"><h2>Telegram API</h2><p>Дані з my.telegram.org для підключення Telethon</p></div></div><div class="accordion-meta">@if($telegramApiConfigured)<span class="status-icon ok" role="img" aria-label="Підключено" title="Підключено">✓</span>@else<span class="status-icon warn" role="img" aria-label="Не налаштовано" title="Не налаштовано">!</span>@endif<span class="accordion-arrow">⌄</span></div></summary>
<div class="accordion-body">
<form method="POST" action="{{ route('alerts.settings.update') }}">@csrf @method('PUT')
<div class="field"><label for="telegram_api_id">API-ідентифікатор</label><input class="input" id="telegram_api_id" name="telegram_api_id" inputmode="numeric" value="{{ old('telegram_api_id', $settings->telegram_api_id) }}" placeholder="Наприклад: 12345678"></div>
@if($settings->telegram_api_hash)
@php($maskedApiHash = substr($settings->telegram_api_hash, 0, 4).str_repeat('•', max(strlen($settings->telegram_api_hash) - 8, 8)).substr($settings->telegram_api_hash, -4))
<div class="field"><label for="telegram_api_hash_saved">Збережений App api_hash</label><input class="input" id="telegram_api_hash_saved" value="{{ $maskedApiHash }}" readonly></div>
@endif
<div class="field"><label for="telegram_api_hash">{{ $settings->telegram_api_hash ? 'Новий App api_hash' : 'App api_hash' }}</label><input class="input" id="telegram_api_hash" type="password" name="telegram_api_hash" placeholder="{{ $settings->telegram_api_hash ? 'Заповніть лише для заміни' : '32 символи з my.telegram.org' }}"></div>
...
<details class="card">
<summary class="accordion-head"><div class="accordion-title"><div class="accordion-copy"><h2>Telegram-бот</h2><p>Простий пульт керування</p></div></div><div class="accordion-meta">@if(in_array($settings->bot_status, ['error', 'failed'], true))<span class="status-icon error" role="img" aria-label="Помилка" title="Помилка">×</span>@elseif($settings->bot_status === 'running')<span class="status-icon ok" role="img" aria-label="Працює" title="Працює">✓</span>@else<span class="status-icon warn" role="img" aria-label="Не налаштовано" title="Не налаштовано">!</span>@endif<span class="accordion-arrow">⌄</span></div></summary>
<div class="accordion-body">
<form method="POST" action="{{ route('alerts.settings.update') }}">@csrf @method('PUT')
...
<div class="field"><label for="source_chat">Канал-джерело</label><input class="input" id="source_chat" name="source_chat" value="{{ old('source_chat', $settings->source_chat) }}" placeholder="@source_channel"></div>
<div class="field"><label for="destination_chat">Група призначення</label><input class="input" id="destination_chat" name="destination_chat" value="{{ old('destination_chat', $settings->destination_chat) }}" placeholder="@destination_group або ID"></div>
<div class="switch-row"><div><strong>Автопублікація</strong></div><label class="switch"><input type="checkbox" name="autopublish_enabled" value="1" @checked(old('autopublish_enabled', $settings->autopublish_enabled))><span class="slider"></span></label></div>
<div class="switch-row"><div><strong>Обробка тексту SkyGuardian</strong></div><label class="switch"><input type="checkbox" name="text_processing_enabled" value="1" @checked(old('text_processing_enabled', $settings->text_processing_enabled))><span class="slider"></span></label></div>
...
<details class="card">
<summary class="accordion-head"><div class="accordion-title"><div class="accordion-copy"><h2>Поточний стан</h2><p>Службова інформація</p></div></div><div class="accordion-meta">@if($settings->last_error || in_array($settings->service_status, ['error', 'failed'], true))<span class="status-icon error" role="img" aria-label="Помилка" title="Помилка">×</span>@elseif($settings->service_status === 'running')<span class="status-icon ok" role="img" aria-label="Працює" title="Працює">✓</span>@else<span class="status-icon warn" role="img" aria-label="Зупинено" title="Зупинено">!</span>@endif<span class="accordion-arrow">⌄</span></div></summary>
