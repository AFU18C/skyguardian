...
<main class="main">
<div class="topbar"><button class="menu-btn" type="button" onclick="document.body.classList.add('menu-open')">☰</button><div class="heading"><h1>Настройки бота тревог</h1><p>Основные параметры Telegram и автопубликации</p></div></div>
<div class="content">
@if(session('status'))<div class="flash">{{ session('status') }}</div>@endif
@if($errors->any())<div class="errors">{{ $errors->first() }}</div>@endif
<div class="grid">
<details class="card">
<summary class="accordion-head"><div class="accordion-title"><div class="accordion-copy"><h2>Технические аккаунты</h2><p>Подключение и управление аккаунтами Telethon</p></div></div><div class="accordion-meta">@if($technicalAccounts->contains('status','error'))<span class="status-icon error">×</span>@elseif($technicalAccounts->contains('status','connected'))<span class="status-icon ok">✓</span>@else<span class="status-icon warn">!</span>@endif<span class="accordion-arrow">⌄</span></div></summary>
...
<details class="card">
<summary class="accordion-head"><div class="accordion-title"><div class="accordion-copy"><h2>Telegram API</h2><p>Данные с my.telegram.org для подключения Telethon</p></div></div><div class="accordion-meta">@if($telegramApiConfigured)<span class="status-icon ok">✓</span>@else<span class="status-icon warn">!</span>@endif<span class="accordion-arrow">⌄</span></div></summary>
...
<details class="card">
<summary class="accordion-head"><div class="accordion-title"><div class="accordion-copy"><h2>Telegram-бот</h2><p>Простой пульт управления</p></div></div><div class="accordion-meta">@if($settings->bot_status === 'running')<span class="status-icon ok">✓</span>@else<span class="status-icon warn">!</span>@endif<span class="accordion-arrow">⌄</span></div></summary>
<div class="accordion-body"><form method="POST" action="{{ route('alerts.settings.update') }}">@csrf @method('PUT')
...
<div class="field"><label for="source_chat">Канал-источник</label><input class="input" id="source_chat" name="source_chat" value="{{ old('source_chat',$settings->source_chat) }}"></div>
<div class="field"><label for="destination_chat">Группа назначения</label><input class="input" id="destination_chat" name="destination_chat" value="{{ old('destination_chat',$settings->destination_chat) }}"></div>
<div class="switch-row"><strong>Автопубликация</strong><label class="switch"><input type="checkbox" name="autopublish_enabled" value="1" @checked(old('autopublish_enabled',$settings->autopublish_enabled))><span class="slider"></span></label></div>
<div class="switch-row"><strong>Обработка текста SkyGuardian</strong><label class="switch"><input type="checkbox" name="text_processing_enabled" value="1" @checked(old('text_processing_enabled',$settings->text_processing_enabled))><span class="slider"></span></label></div>
<div class="savebar"><button class="btn" type="submit">Сохранить настройки</button></div></form></div>
</details>

<details class="card">
<summary class="accordion-head"><div class="accordion-title"><div class="accordion-copy"><h2>Текущее состояние</h2><p>Служебная информация</p></div></div><div class="accordion-meta">@if($settings->last_error)<span class="status-icon error">×</span>@elseif($settings->service_status === 'running')<span class="status-icon ok">✓</span>@else<span class="status-icon warn">!</span>@endif<span class="accordion-arrow">⌄</span></div></summary>
